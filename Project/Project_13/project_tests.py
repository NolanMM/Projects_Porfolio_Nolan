...
df = pd.read_csv('./data/Project_13/user-item-interactions.csv')
df_content = pd.read_csv('./data/Project_13/articles_community.csv')
del df['Unnamed: 0']
...
def sol_2_test(top_articles):
    ...
    for idx, file in enumerate(checks):
        if set(eval(file)) == set(pickle.load(open("{}.p".format(file), "rb"))):
            ...
            print(
                "Oops! The {} list doesn't look how we expected.  Try again.".format(file))
    ...
    if sol_4_dict == sol_4_dict_1:
        ...
        for k, v in sol_4_dict_1.items():
            if sol_4_dict_1[k] != sol_4_dict[k]:
                print(
                    "Sorry it looks like that isn't the right value associated with {}.  Try again.".format(k))
